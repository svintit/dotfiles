function __fuzzy_cd -d "fuzzy cd with jump bookmarks"
	function __fuzzy_cd_chdir
		...
		if test (string match --regex '\.{3,}' $first_token)
			...
			set res (fcd_ffdir -i -d4 --multi --shortest $base $token)
		else if $try_mark
		...
		set -l result (printf "%s\n" $res | nl -w1 -s'. ' | fzf -1 -0 --height=40% --border=rounded --prompt='📁 ' --pointer='▶' --marker='✓' --color='border:#00D9FF,prompt:#00D9FF,pointer:#FF79C6,marker:#50FA7B' --reverse --tiebreak=begin,length | sed 's/^[0-9]*\. //')

		set result (string trim $result)

		if test -n "$result"; and test "$result" != "."
