function fcd_ffdir -d "fuzzy find a directory, pass root dir and sequential search strings"
	...
	if test (count $args) -gt 0
		...
		test $max_depth -gt 8 && set max_depth 8
		set -l found "."
		# if search string contains uppercase, make search case sensitive
		if set -q _flag_i
			set case_sensitive "iregex"
		else if string match -q --regex [A-Z] $args or set -q _flag_c
		...
		set -l results (find -E -L -s "$new_path" -$case_sensitive ".*/[^.]*$regex.*" -type d -maxdepth $max_depth -mindepth 1 2> /dev/null)
		...
		if test -n "$found"
			...
			set results (find -E -L -s "$new_path" -$case_sensitive ".*$regex.*" -type d -maxdepth $max_depth -mindepth 1 2> /dev/null)
			if set -q _flag_shortest
