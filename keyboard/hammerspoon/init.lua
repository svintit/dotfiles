...
-- Keep the Spotify mini player on BenQ, or use the built-in display as fallback.
...
local spotifyMiniMinWidth = 240
local spotifyMiniMaxWidth = 700
local spotifyMiniChromeWidth = 160
local spotifyMiniWidth = 650
local spotifyMiniHeight = 52
local spotifyMiniPadding = 9
local spotifyMiniDelay = 3
local screenEdgeInset = 50
local spotifyPositioning = {}
local spotifyPositionResetTimers = {}
local spotifyPositionTimers = {}
local spotifyTrackTask = nil
local spotifyTrackKey = nil
local spotifyFilter = hs.window.filter.new({"Spotify"})


local function spotifyMiniTarget(screens)
    local builtIn = nil
    local benq = nil
    local officeDell = nil
    for _, screen in ipairs(screens) do
        local name = screen:name()
        if name == builtInDisplayName then
            builtIn = screen
        elseif name == benqDisplayName then
            benq = screen
        elseif name == officeDellDisplayName then
            officeDell = screen
        end
    end
    if benq then return benq, false end
    if officeDell then return officeDell, false end
    return builtIn, true
end
local function spotifyMiniWidthForTrack(title, artist)
    local canvas = _G.desktopClocks and _G.desktopClocks[1]
    local function measure(text)
        if canvas then return canvas:minimumTextSize(text).w end
        return hs.drawing.getTextDrawingSize(text).w
    end
    local titleText = hs.styledtext.new(title, {
        font = {name = "HelveticaNeue", size = 13},
    })
    local artistText = hs.styledtext.new(artist, {
        font = {name = "HelveticaNeue", size = 11},
    })
    local titleWidth = measure(titleText)
    local artistWidth = measure(artistText)
    local contentWidth = math.max(titleWidth, artistWidth) + spotifyMiniChromeWidth
    return math.max(spotifyMiniMinWidth, math.min(spotifyMiniMaxWidth, math.ceil(contentWidth)))
end

_G.spotifyMiniWidthForTrack = spotifyMiniWidthForTrack

local spotifyPinTasks = {}

local function runSpotifyPin(callback, windowId)
    local task, timeout
    ...
    local function finish(success)
        ...
        if task then spotifyPinTasks[task] = nil end
        callback(success)
    end
    task = hs.task.new(os.getenv("HOME") .. "/.local/bin/aerospace-spotify-pin", function(code, _, stderr)
        if code ~= 0 then print("Spotify positioning failed: " .. tostring(stderr)) end
        finish(code == 0)
    end, windowId and {tostring(windowId)} or {})
    if not task then finish(false); return end
    spotifyPinTasks[task] = true
    if not task:start() then finish(false); return end
    timeout = hs.timer.doAfter(5, function()
        print("Spotify positioning timed out")
        finish(false)
        task:terminate()
    end)
    spotifyPinTasks[task] = timeout
end

local function positionSpotifyMiniPlayer(win)
    if not isSpotifyMiniPlayer(win) then return end
    local targetScreen, useBuiltInLayout = spotifyMiniTarget(hs.screen.allScreens())
    local winId = win:id()
    if not targetScreen or not winId or spotifyPositioning[winId] then return end
    local width = math.max(spotifyMiniMinWidth, spotifyMiniWidth)
    local targetUUID = targetScreen:getUUID()
    local screenFrame = targetScreen:frame()
    local x = screenFrame.x + screenEdgeInset
    local y = screenFrame.y + spotifyMiniPadding
    if not useBuiltInLayout then
        x = screenFrame.x + (screenFrame.w - width) / 2
    elseif useBuiltInLayout then
        local fullFrame = targetScreen:fullFrame()
        x = fullFrame.x + screenEdgeInset
        y = fullFrame.y + spotifyMiniPadding
    end
    local targetFrame = {
        x = x,
        y = y,
    ...
    local frame = win:frame()
    if math.abs(frame.x - targetFrame.x) < 1
        and math.abs(frame.y - targetFrame.y) < 1
        and math.abs(frame.w - targetFrame.w) < 1
        and math.abs(frame.h - targetFrame.h) < 1 then
        return
    end
    spotifyPositioning[winId] = true
    runSpotifyPin(function(success)
        local currentWindow = success and hs.window.get(winId)
        local currentScreen = spotifyMiniTarget(hs.screen.allScreens())
        if not currentWindow or not isSpotifyMiniPlayer(currentWindow)
            or not currentScreen or currentScreen:getUUID() ~= targetUUID then
            spotifyPositioning[winId] = nil
            return
        end
        currentWindow:setFrame(targetFrame, 0.3)
        spotifyPositionResetTimers[winId] = hs.timer.doAfter(0.5, function()
            spotifyPositioning[winId] = nil
            spotifyPositionResetTimers[winId] = nil
        end)
    end, winId)
end

local function scheduleSpotifyMiniPlayer(win)
    if not isSpotifyMiniPlayer(win) then return end
    local winId = win:id()
    if not winId or spotifyPositioning[winId] then return end
    if spotifyPositionTimers[winId] then
        spotifyPositionTimers[winId]:stop()
    end
    -- AeroSpace can steal the window to the focused monitor; snap back at once.
    local targetScreen = spotifyMiniTarget(hs.screen.allScreens())
    if targetScreen and win:screen() ~= targetScreen then
        positionSpotifyMiniPlayer(win)
        return
    end
    spotifyPositionTimers[winId] = hs.timer.doAfter(spotifyMiniDelay, function()
        spotifyPositionTimers[winId] = nil
        positionSpotifyMiniPlayer(win)
    end)
end
local function refreshSpotifyMiniWidth()
    if spotifyTrackTask or not hs.application.find("Spotify") then return end

    spotifyTrackTask = hs.task.new("/usr/bin/osascript", function(exitCode, stdout)
        spotifyTrackTask = nil
        if exitCode ~= 0 then return end

        local title, artist = stdout:gsub("\r", ""):match("^([^\n]+)\n([^\n]+)")
        if not title or not artist then return end
        local trackKey = title .. "\n" .. artist
        local trackChanged = trackKey ~= spotifyTrackKey
        local nextWidth = spotifyMiniWidthForTrack(title, artist)
        local widthChanged = nextWidth ~= spotifyMiniWidth

        spotifyTrackKey = trackKey
        spotifyMiniWidth = nextWidth
        local app = hs.application.find("Spotify")
        if not app then return end
        for _, win in ipairs(app:allWindows()) do
            local frame = win:frame()
            local wrongWidth = math.abs(frame.w - nextWidth) >= 1
            if trackChanged or widthChanged or wrongWidth then
                positionSpotifyMiniPlayer(win)
            end
        end
    end, {
    ...
    spotifyTrackTask:start()
end

spotifyFilter:subscribe(hs.window.filter.windowCreated, scheduleSpotifyMiniPlayer)
spotifyFilter:subscribe(hs.window.filter.windowMoved, scheduleSpotifyMiniPlayer)
-- Spotify hides its web UI from accessibility until these attributes are set.
-- Never reset them to false: Spotify tears the tree down and, verified
-- 2026-08-28, never rebuilds it until the app restarts.
local function pressOpenMiniplayer()
    ...
    local function tryPress()
        ...
        local function walk(el, depth)
            if pressed or depth > 40 or visited > 20000 then return end
            visited = visited + 1
            for _, kid in ipairs(el:attributeValue("AXChildren") or {}) do
                local label = (kid:attributeValue("AXDescription") or "")
                    .. (kid:attributeValue("AXTitle") or "")
                if string.find(string.lower(label), "open miniplayer") then
                    kid:performAction("AXPress")
                    pressed = true
                    return
                end
                walk(kid, depth + 1)
    ...
    -- The tree appears with a lag after the flags flip; retry until it does.
    local attempts = 0
    local function attempt()
        attempts = attempts + 1
        if tryPress() or attempts >= 10 then return end
        hs.timer.doAfter(0.5, attempt)
    end
    attempt()
end

_G.pressOpenMiniplayer = pressOpenMiniplayer

local spotifyEnsureTimer
local spotifyLaunchCheckTimer
local spotifyRelaunchTimer
local spotifyAccessibilityRelaunchPending = false
local spotifyHasAccessibilityTree

local function relaunchSpotifyWithAccessibility(app)
    if spotifyAccessibilityRelaunchPending then return end
    spotifyAccessibilityRelaunchPending = true
    app:kill()
    spotifyRelaunchTimer = hs.timer.doAfter(3, function()
        hs.execute("/usr/bin/open -g -a Spotify --args --force-renderer-accessibility")
    end)
end

local function ensureSpotifyMiniPlayer()
    local app = hs.application.find("Spotify")
    if not app then return end
    for _, win in ipairs(app:allWindows()) do
        if isSpotifyMiniPlayer(win) then
            spotifyAccessibilityRelaunchPending = false
            return
        end
    end
    if spotifyHasAccessibilityTree() then
        spotifyAccessibilityRelaunchPending = false
        pressOpenMiniplayer()
        return
    end
    if spotifyAccessibilityRelaunchPending then
        pressOpenMiniplayer()
        return
    end
    relaunchSpotifyWithAccessibility(app)
end

spotifyFilter:subscribe(hs.window.filter.windowDestroyed, function()
    if spotifyEnsureTimer then spotifyEnsureTimer:stop() end
    spotifyEnsureTimer = hs.timer.doAfter(1.5, ensureSpotifyMiniPlayer)
end)
spotifyFilter:subscribe(hs.window.filter.windowCreated, function(win)
    if isSpotifyMiniPlayer(win) then
        spotifyAccessibilityRelaunchPending = false
    end
end)

-- The miniplayer button needs Chromium renderer accessibility.
spotifyHasAccessibilityTree = function()
    local axuielement = require("hs.axuielement")
    local app = hs.application.find("Spotify")
    ...
    local found = false
    local function walk(el, depth)
        if found or visited > 20000 or depth > 40 then return end
        visited = visited + 1
        local label = (el:attributeValue("AXDescription") or "")
            .. (el:attributeValue("AXTitle") or "")
        if label:lower():find("open miniplayer", 1, true) then
            found = true
            return
        end
        for _, kid in ipairs(el:attributeValue("AXChildren") or {}) do
...
local spotifyLaunchWatcher = hs.application.watcher.new(function(name, event)
    if name ~= "Spotify" or event ~= hs.application.watcher.launched then return end
    spotifyLaunchCheckTimer = hs.timer.doAfter(10, ensureSpotifyMiniPlayer)
end)
_G.spotifyLaunchWatcher = spotifyLaunchWatcher
spotifyLaunchWatcher:start()
spotifyEnsureTimer = hs.timer.doAfter(1.5, function()
    local app = hs.application.find("Spotify")
    if not app then return end
    for _, win in ipairs(app:allWindows()) do
        if isSpotifyMiniPlayer(win) then
            scheduleSpotifyMiniPlayer(win)
            return
        end
    end
    pressOpenMiniplayer()
end)
_G.positionSpotifyMiniPlayer = scheduleSpotifyMiniPlayer
_G.spotifyMiniTarget = spotifyMiniTarget
local spotifyScreenChangeTimer
local spotifyScreenWatcher = hs.screen.watcher.new(function()
    if spotifyScreenChangeTimer then spotifyScreenChangeTimer:stop() end
    spotifyScreenChangeTimer = hs.timer.doAfter(1, function()
        spotifyScreenChangeTimer = nil
        runSpotifyPin(function(success)
...
_G.spotifyScreenWatcher = spotifyScreenWatcher
spotifyScreenWatcher:start()
local spotifyTrackTimer = hs.timer.doEvery(2, refreshSpotifyMiniWidth)
_G.spotifyTrackTimer = spotifyTrackTimer
refreshSpotifyMiniWidth()
